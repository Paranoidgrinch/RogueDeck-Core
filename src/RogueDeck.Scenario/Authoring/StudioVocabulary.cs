...
public static class StudioVocabulary
{
    ...
    private static readonly IReadOnlyDictionary<string, string> NodeDescriptions = new Dictionary<string, string>
    {
        ...
        ["markCardInstance"] = "Put a mark on ONE COPY of a card (or take it off). The mark travels with that copy through every zone, so a rule can find it again later.",
        ["setCardInstanceMarkCounter"] = "Write a number on ONE COPY of a card. The reserved counter 'standard.card_cost_delta' changes what that copy costs the next time it is played.",
    ...
    [
        ...
        ("Cards", ["drawCards", "moveCards", "moveCardToZone", "transformCard", "createCardInstance",
            ...
            "markCardInstance", "setCardInstanceMarkCounter"]),
    ...
    private static readonly IReadOnlyDictionary<string, string> EnumLabels = new Dictionary<string, string>
    {
        ...
        ["StatusData.Amplification"] = "Amplification",
        ["StatusData.CombatRules"] = "Decrees",
        ["PassiveModifierOperation.ClampMax"] = "cap it at this",
        ["CombatRule.MaxCardsPerTurn"] = "no more than N cards a turn",
        ["CombatRule.NoLikenessInSuccession"] = "never the same kind twice in a row",
        ["CombatRule.NthCardOfTurnIsFree"] = "the Nth card each turn is free",
        ["CombatRule.MinimumCardCost"] = "no card may cost less than N",
        ["CombatRule.UnspentResourceCarries"] = "what is unspent carries over",
        ["CombatRule.PlayedCardsExhaust"] = "a played card exhausts",
        ["CombatRule.MaxHandSize"] = "the hand holds no more than N",
        ["StatusAmplificationScope.Any"] = "anything applied to the bearer",
        ["StatusAmplificationScope.Debuffs"] = "debuffs",
        ["StatusAmplificationScope.Buffs"] = "buffs",
        ["StatusAmplificationScope.UnwantedByBearer"] = "what the bearer would not want",
        ...
        ["TriggerEvent.StatusApplicationAmplified"] = "a status application is made larger",
    ...
    private static readonly IReadOnlyDictionary<string, string> EnumDescriptions = new Dictionary<string, string>
    {
        ...
        ["StatusData.Amplification"] = "While this status is worn, the next status applied to its bearer lands larger, and the amplification is spent doing it. It is the mirror of a prohibition, and it is polarity-blind by default: the bearer can spend it deliberately on a blessing of their own instead of letting it magnify the next curse. An amplification never enlarges itself, and never enlarges one application twice.",
        ["StatusData.CombatRules"] = "Rules of combat that are DIFFERENT while this status is worn — not a number changed but a rule replaced: how many cards a turn may hold, what a card costs for where it falls in the turn, where a played card goes, how much a hand may hold. Each rule is read about the combatant wearing it, so a rule meant to bind both sides is worn by both.",
        ["PassiveModifierOperation.ClampMax"] = "A ceiling: the amount is reduced to the magnitude if it was above it, and left alone if it was not. Neither adding nor scaling can say this — 'no single blow above twenty' is not twenty less and not halved.",
        ...
        ["TriggerEvent.StatusApplicationAmplified"] = "Fires when an amplification enlarges an incoming status application and is spent doing it.",
    ...
    public static string Describe(CombatNodeModel node)
    {
        ...
        return node.Kind switch
        {
            ...
            "markCardInstance" => $"{(node.Relative ? "unmark" : "mark")} a card of {target} as '{node.ToTag}'",
            "setCardInstanceMarkCounter" => $"set '{node.CounterId}' to {amount} on a card of {target}",
