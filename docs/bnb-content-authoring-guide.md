...
| **"At the start of your turn, with your hand in front of you"** (relics, inscriptions, keywords) | The hand is dealt AFTER the turn-start triggers run, so the rule hangs on `CardsDrawn` — and must be guarded with `cardDrawsThisTurn(hero) <= 1`. Unguarded it also fires for every card any other rule or card draws; and if it pays in CARDS, its own payment is another draw, so it pays itself. Several such relics in one deck is how a run died in Act IV. |
